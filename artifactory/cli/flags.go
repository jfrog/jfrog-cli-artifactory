...
var flagsMap = map[string]components.Flag{
	...
	// AccessTokenCreate
	atcProject:     components.NewStringFlag(Project, "[Optional] The project for which this token is created. Enter the project name on which you want to apply this token.", components.SetMandatoryFalse()),
	atcGrantAdmin:  components.NewBoolFlag(GrantAdmin, "[Default: false] Set to true to provide admin privileges to the access token. This is only available for administrators.", components.WithBoolDefaultValueFalse()),
	atcGroups:      components.NewStringFlag(Groups, "[Optional] A list of comma-separated(,) groups for the access token to be associated with. This is only available for administrators.", components.SetMandatoryFalse()),
	atcScope:       components.NewStringFlag(Scope, "[Optional] The scope of access that the token provides. This is only available for administrators.", components.SetMandatoryFalse()),
	atcExpiry:      components.NewStringFlag(Expiry, "[Optional] The amount of time, in seconds, it would take for the token to expire. Must be non-negative. If not provided, the platform default will be used. To specify a token that never expires, set to zero. Non-admin may only set a value that is equal or lower than the platform default that was set by an administrator (1 year by default).", components.SetMandatoryFalse()),
	atcRefreshable: components.NewBoolFlag(Refreshable, "[Default: false] Set to true if you'd like the token to be refreshable. A refresh token will also be returned in order to be used to generate a new token once it expires.", components.WithBoolDefaultValueFalse()),
	atcDescription: components.NewStringFlag(Description, "[Optional] Free text token description. Useful for filtering and managing tokens. Limited to 1024 characters.", components.SetMandatoryFalse()),
	...
	// TransferFiles
	Filestore:           components.NewBoolFlag(Filestore, "[Default: false] Set to true to make the transfer mechanism check for the existence of artifacts on the target filestore. Used when the files are already expected to be located on the filestore.", components.WithBoolDefaultValueFalse()),
	IgnoreState:         components.NewBoolFlag(IgnoreState, "[Default: false] Set to true to ignore the saved state from previous transfer-files operations.", components.WithBoolDefaultValueFalse()),
	ProxyKey:            components.NewStringFlag(ProxyKey, "[Optional] The key of an HTTP proxy configuration in Artifactory. This proxy will be used for the transfer traffic between the source and target instances. To configure this proxy, go to \"Proxies | Configuration | Proxy Configuration\" in the JFrog Administration UI.", components.SetMandatoryFalse()),
	transferFilesStatus: components.NewBoolFlag(Status, "[Default: false] Set to true to show the status of the transfer-files command currently in progress.", components.WithBoolDefaultValueFalse()),
	Stop:                components.NewBoolFlag(Stop, "[Default: false] Set to true to stop the transfer-files command currently in progress. Useful when running the transfer-files command in the background.", components.WithBoolDefaultValueFalse()),
}
