...
// repoListRow is one row for registry mode (jf agent skills list --repo).
...
// localListRow is one row for local mode (jf agent skills list --harness).
...
// ErrSkillsListNoMode is returned when neither --repo nor --harness is set.
func ErrSkillsListNoMode() error {
	return fmt.Errorf(
		"jf agent skills list requires exactly one of:\n"+
			"  Registry:  jf agent skills list --repo <repository-key> [--limit N] [--sort-by updated|downloads]\n"+
			"  Project:   jf agent skills list --harness <name> [--project-dir <path>]   (default --project-dir: current directory)\n"+
			"  Global:    jf agent skills list --harness <name> --global\n"+
			"\n"+
			"Notes:\n"+
			"  --repo and --harness are mutually exclusive.\n"+
			"  With --harness, --global and --project-dir are mutually exclusive.\n"+
...
// checkUpdateStatusFromSemverComparison maps agentcommon.CompareSemver numeric result to a list STATUS value.
...
func (lc *ListCommand) fillUpdateStatus(row *localListRow, slug string) {
	...
	semverComparison, err := agentcommon.CompareSemver(row.Version, latest)
	if err != nil {
		row.Status = listCheckStatusUnknown
		return
	}
	...
	if repoKey != "" {
		...
		if rawSortBy := strings.ToLower(c.GetStringFlagValue("sort-by")); rawSortBy != "" {
			if rawSortBy != sortByName {
				return fmt.Errorf("--sort-by for --harness only accepts 'name', got: %q", rawSortBy)
			}
			sortBy = rawSortBy
		}
