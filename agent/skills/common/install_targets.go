...
// HandleEvidenceVerification runs evidence verification for a skill using skill-specific policy.
func HandleEvidenceVerification(quiet bool, slug string, verifyFn func() error) error {
	return agentcommon.HandleEvidenceVerification(quiet, slug, ArtifactKind, verifyFn,
		agentcommon.ShouldFailOnMissingEvidenceForSkills,
		agentcommon.DisableQuietFailureEvidenceHintForSkills)
}
