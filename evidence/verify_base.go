...
type verifyEvidenceBase struct {
	...
	verifier           cryptox.EvidenceVerifierInterface
}
...
func (v *verifyEvidenceBase) verifyEvidences(client *artifactory.ArtifactoryServicesManager, evidenceMetadata *[]model.SearchEvidenceEdge, sha256, subjectPath string) error {
	if v.verifier == nil {
		v.verifier = cryptox.NewEvidenceVerifier(v.keys, v.useArtifactoryKeys, client)
	}
