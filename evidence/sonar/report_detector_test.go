...
func TestGetReportTaskPathWithConfigFile(t *testing.T) {
	// Test with actual config file
	configContent := `sonar:
  reportTaskFile: "custom/path/report-task.txt"`

	// Create config file
	configDir := ".jfrog/evidence"
	if err := os.MkdirAll(configDir, 0755); err != nil {
		t.Fatalf("Failed to create config directory %s: %v", configDir, err)
	}
	configFile := filepath.Join(configDir, "evidence.yml")
	if err := os.WriteFile(configFile, []byte(configContent), 0644); err != nil {
		t.Fatalf("Failed to create config file %s: %v", configFile, err)
	}

	// Create the configured file
	configuredPath := "custom/path/report-task.txt"
	configuredDir := filepath.Dir(configuredPath)
	if err := os.MkdirAll(configuredDir, 0755); err != nil {
		t.Fatalf("Failed to create directory %s: %v", configuredDir, err)
	}
	if err := os.WriteFile(configuredPath, []byte("test content"), 0644); err != nil {
		t.Fatalf("Failed to create test file %s: %v", configuredPath, err)
	}
	...
	if err := os.RemoveAll(configDir); err != nil {
		t.Logf("Warning: failed to cleanup %s: %v", configDir, err)
	}
	if err := os.RemoveAll(configuredDir); err != nil {
		t.Logf("Warning: failed to cleanup %s: %v", configuredDir, err)
	}
	if err := os.RemoveAll(standardDir); err != nil {
		t.Logf("Warning: failed to cleanup %s: %v", standardDir, err)
	}
...
func TestGetReportTaskPathWithInvalidConfigFile(t *testing.T) {
	// Test with config file pointing to non-existent path
	configContent := `sonar:
  reportTaskFile: "non/existent/path/report-task.txt"`
	...
	// Create a standard candidate file for fallback
	standardPath := "target/sonar/report-task.txt"
	standardDir := filepath.Dir(standardPath)
	if err := os.MkdirAll(standardDir, 0755); err != nil {
		t.Fatalf("Failed to create directory %s: %v", standardDir, err)
