...
func NewVerifyEvidencesBuild(serverDetails *coreConfig.ServerDetails, project, buildName, buildNumber, format string, keys []string) Command {
...
func (v *verifyEvidenceBuild) Run() error {
	...
	repoKey := buildBuildInfoRepoKey(v.project)
	result, err := ExecuteAqlQuery(fmt.Sprintf(aqlBuildQueryTemplate, repoKey, v.buildName, v.buildNumber), client)
	if err != nil {
	...
	return v.verifyEvidences(client, metadata, buildInfoSha256)
}

// ServerDetails returns the server details for the command.
func (v *verifyEvidenceBuild) ServerDetails() (*coreConfig.ServerDetails, error) {
