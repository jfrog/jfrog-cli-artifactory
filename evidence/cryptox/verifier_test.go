...
func (m *MockArtifactoryServicesManagerVerifier) GetTrustedKeys() (*services.TrustedKeysResponse, error) {
	// Return empty response to avoid method not implemented error
	return &services.TrustedKeysResponse{}, nil
}

func (m *MockArtifactoryServicesManagerVerifier) GetKeyPairs() (*[]services.KeypairResponseItem, error) {
	// Return empty response to avoid method not implemented error
	return &[]services.KeypairResponseItem{}, nil
}
...
func (m *MockArtifactoryServicesManagerVerifierStrict) GetTrustedKeys() (*services.TrustedKeysResponse, error) {
	// This should NOT be called when useArtifactoryKeys=false
	m.t.Fatal("GetTrustedKeys() should not be called when useArtifactoryKeys=false")
	return nil, nil
}
...
// Test NewEvidenceVerifier constructor
func TestNewVerifier(t *testing.T) {
	...
	verifier := NewEvidenceVerifier(keys, true, &clientInterface)
...
// Test NewEvidenceVerifier with nil client and details (tests graceful handling)
func TestNewVerifier_NilClientAndDetails(t *testing.T) {
	keys := []string{"key1"}

	// The NewEvidenceVerifier function dereferences client and details pointers, so passing nil will panic
	// This test documents this behavior
	defer func() {
		if r := recover(); r != nil {
			// Expected behavior - NewEvidenceVerifier panics with nil pointers
	...
	NewEvidenceVerifier(keys, true, nil)
}

// Test NewEvidenceVerifier with empty slices
...
// Test NewEvidenceVerifier with nil keys and keyIds but valid client and details
...
func TestVerifier_Verify_Success(t *testing.T) {
	...
	assert.Equal(t, "test-sha256", evidence.Checksum)
