...
func getPublishCommand() components.Command {
	return components.Command{
		...
		Description: "Publish an AI agent plugin to Artifactory. Discovers plugin.json under the given directory (root and known agent subdirs), validates that all manifests agree on name and version, and uploads a zip to a local repository of package type 'agentplugins'. For marketplace mono-repos, pass plugins/<plugin-name>/ rather than the repo root. Version precedence: --version flag, then consensus from manifests, then default 1.0.0.",
