...
func (ec *EvidenceCreateCommand) Run() error {
	...
	// Load all the private keys from a folder
	keyFiles, err := os.ReadDir(ec.key)
	if err != nil {
		return err
	}

	privateKeys, err := cryptolib.ReadKey(keyFiles, ec.key)
	if err != nil {
		return err
	}
	// If keyId is provided, use it to the single key in the privateKeys slice
	if ec.keyId != "" {
		(*privateKeys)[0].KeyID = ec.keyId
	}

	var signers []dsse.Signer
	for _, key := range *privateKeys {
		// create actual singers
		if key.KeyType == cryptolib.ECDSAKeyType {
			ecdsaSinger, err := cryptolib.NewECDSASignerVerifierFromSSLibKey(key)
			if err != nil {
				return err
			}
			signers = append(signers, ecdsaSinger)
		} else if key.KeyType == cryptolib.RSAKeyType {
			rsaSinger, err := cryptolib.NewRSAPSSSignerVerifierFromSSLibKey(key)
			if err != nil {
				return err
			}
			signers = append(signers, rsaSinger)
		} else if key.KeyType == cryptolib.ED25519KeyType {
			ed25519Singer, err := cryptolib.NewED25519SignerVerifierFromSSLibKey(key)
			if err != nil {
				return err
			}
	...
	// Create the evidencecore file.
	evdName := "/evidencecore.json.evd"
	if ec.evidenceName != "" {
