...
func TestED25519SignerVerifierVerify(t *testing.T) {
	...
	assert.Nil(t, sv.Verify(message, signature))

	message = []byte("corrupted message")
	err = sv.Verify(message, signature)
...
//	}
//
//	env, err := es.SignPayload(payloadType, payload)
//	if err != nil {
//		t.Error(err)
//	}
//
...
//	}
//
//	acceptedKeys, err := ev.Verify(env)
//	assert.Nil(t, err)
